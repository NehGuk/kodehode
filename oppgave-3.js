...
for (let i = 0; i < people.length; i++) {
  ...
  console.log(
    `${people[i].title} ${people[i].name} is from ${people[i].city}. ${people[i].name} is ${people[i].age} years old and enjoys ${people[i].hobbies.join(", ")}...`
  )
}

console.log(`The combined age of all these folks is ${combinedAge}. `)

// PART 7
averageAge = Math.floor(combinedAge / (people.length - 1))
console.log(`Their average age is ${averageAge}.`)
...
function diceRoller(throws, sides) {
  let result = []
  for (let i = 1; i <= throws; i++) {
    result.push((randomNumber = Math.ceil(Math.random() * sides)))
  }
  return result
  console.log(`You've thrown a ${sides}-sided dice ${throws} times.`)
}
...
function cleanUpWords(arrayOfMessyStrings) {
  let cleanedUpText = []
  for (word of arrayOfMessyStrings) {
    cleanedUpText.push((word = word.trim().toLowerCase()))
  }
  return cleanedUpText.join(" ")
}
...
function doubleSwap(string, charA, charB) {
  ...
  for (let char of string) {
    if (char !== charA && char !== charB) {
      ...
      decodedString = decodedString + charB
    } else if (char === charB) {
      decodedString = decodedString + charA
    }
  }
...
function helloChecker(string) {
  if (string.toLowerCase().includes("hello")) {
    return "HELLO oppdaged på engelsk."
  }
  if (string.toLowerCase().includes("ciao")) {
    return "HELLO oppdaged på italiensk."
  }
  if (string.toLowerCase().includes("salut")) {
    return "HELLO oppdaged på fransk."
  }
  if (string.toLowerCase().includes("hallo")) {
    return "HELLO oppdaged på tysk."
  }
  if (string.toLowerCase().includes("hola")) {
    return "HELLO oppdaged på spansk."
  }
  if (string.toLowerCase().includes("cześć")) {
    return "HELLO oppdaged på polsk."
  } else {
